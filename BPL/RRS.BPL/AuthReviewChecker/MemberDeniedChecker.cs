...
{
    public class MemberDeniedChecker : IAuthReviewChecker
    {
        public ValidationResult CheckAuthReview(AuthRequest request, Member member, Provider provider, ref List<PendCode> pendList)
        {
            ...
            try
            {
                var pc = PendCodeAdapter.CheckMemberDenied(request, member, 6);
            ...
            {
                var errMsg = "Error checking member been denied due to: " + ex.Message;
