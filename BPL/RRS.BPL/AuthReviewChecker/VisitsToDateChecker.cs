...
{
    public class VisitsToDateChecker : IAuthReviewChecker
    {
        public ValidationResult CheckAuthReview(AuthRequest request, Member member, Provider provider, ref List<PendCode> pendList)
        {
            ...
            try
            {
                var pc = PendCodeAdapter.CheckVisitsToDate(request, member);
            ...
            {
                var errMsg = "Error checking visits to date to plan limit due to: " + ex.Message;
