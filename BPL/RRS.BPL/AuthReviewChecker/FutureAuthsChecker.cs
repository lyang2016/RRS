...
{
    public class FutureAuthsChecker : IAuthReviewChecker
    {
        public ValidationResult CheckAuthReview(AuthRequest request, Member member, Provider provider, ref List<PendCode> pendList)
        {
            ...
            try
            {
                var pc = PendCodeAdapter.CheckFutureAuths(request, member);
            ...
            {
                var errMsg = "Error checking member future auths due to: " + ex.Message;
