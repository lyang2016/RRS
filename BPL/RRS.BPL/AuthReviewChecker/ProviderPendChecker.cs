...
{
    public class ProviderPendChecker : IAuthReviewChecker
    {
        public ValidationResult CheckAuthReview(AuthRequest request, Member member, Provider provider, ref List<PendCode> pendList)
        {
            ...
            try
            {
                var pc = PendCodeAdapter.CheckProviderPend(provider);
            ...
            {
                var errMsg = "Error checking provider pended due to: " + ex.Message;
